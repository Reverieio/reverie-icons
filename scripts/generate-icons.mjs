#!/usr/bin/env node
/**
 * Reverie Icons — SVG-to-Code Generator
 *
...
const TSUP_CFG   = resolve(ROOT, 'packages/core/tsup.config.ts')
 ...
 *
 * - outline: force all → single strokeLayer (primary)
 * - solid:   force all → single fillLayer (primary)
...
function buildLayers(elements, variation) {
  ...
  for (const el of elements) {
    ...
    // Heuristic: in duotone, filled elements with opacity < 1 → secondary
...
// ── Auto-Generation of Missing Variations ──────────────────────────────────
...
// ── Code Generation: Barrel Exports ────────────────────────────────────────
...
function generateCoreIndex(iconNames) {
  ...
  code += ` * This package contains pure data — no framework dependencies.\n */\n\n`
  ...
  // Builders
  code += `// ── Builders (for creating custom icons) ────────────────────────────────────\n`
  code += `export {\n  path,\n  circle,\n  rect,\n  line,\n  polyline,\n  polygon,\n  ellipse,\n  strokeLayer,\n  fillLayer,\n  defineIcon,\n} from './define'\n\n`
  ...
  // Registry
  code += `// ── Registry (all icons in one map) ─────────────────────────────────────────\n`
  code += `import type { IconSet } from './types'\n\n`
  for (const name of iconNames) {
...
function generateVueIndex(iconNames) {
  ...
  code += `// ── Pre-built icon components ───────────────────────────────────────────────\n\n`
...
// ── Code Generation: tsup Config ──────────────────────────────────────────

function generateTsupConfig(iconNames) {
  const entries = iconNames.map(n => `    'icons/${n}': 'src/icons/${n}.ts',`).join('\n')
...
function main() {
  console.log('🎨 Reverie Icons — SVG-to-Code Generator\n')
  ...
  if (existsSync(META_JSON)) {
    ...
    console.log(`  📄 Loaded ${Object.keys(metadata).length} metadata entries from config/icon-metadata.json`)
  }
  ...
  if (existsSync(SVGS_DIR)) {
    ...
    if (folders.length > 0) {
      console.log(`\n  📂 Found ${folders.length} icon folder(s) in svgs/\n`)
    }
    ...
    for (const folder of folders) {
      ...
      if (!parsed.outline) {
        console.log(`  ⚠️  Skipping ${folder}/ — no outline.svg found`)
        continue
      }
      ...
      console.log(`  📦 ${camelName} (${displayN})`)
      ...
      if (existsSync(metaPath)) {
        try {
          ...
          console.log(`     ⚠️  Invalid meta.json — using defaults`)
        }
      }
    ...
    if (generatedCount > 0) {
      console.log(`\n  ✨ Generated ${generatedCount} icon(s) from SVG files`)
    } else if (folders.length === 0) {
      console.log(`  ℹ️  No icon folders in svgs/ — skipping SVG generation`)
    }
  } else {
    console.log('  ℹ️  No svgs/ directory found — skipping SVG generation')
  }
  ...
  console.log(`\n  📋 Total icons: ${allIconNames.length} (${generatedCount} generated, ${allIconNames.length - generatedCount} hand-coded)`)
  ...
  for (const key of Object.keys(metadata)) {
    if (!allIconNames.includes(key)) {
      console.log(`  🗑️  Removing stale metadata: ${key}`)
      delete metadata[key]
    }
  }
  ...
  if (!DRY_RUN) {
    ...
    console.log(`     ✅ config/icon-metadata.json`)

    // Core barrel
    writeFileSync(join(CORE_SRC, 'index.ts'), generateCoreIndex(allIconNames))
    console.log(`     ✅ packages/core/src/index.ts`)

    // Core metadata
    writeFileSync(join(CORE_SRC, 'metadata.ts'), generateMetadataTS(sortedMeta, categories))
    console.log(`     ✅ packages/core/src/metadata.ts`)

    // React barrel
    writeFileSync(join(REACT_SRC, 'index.ts'), generateReactIndex(allIconNames))
    console.log(`     ✅ packages/react/src/index.ts`)

    // Vue barrel
    writeFileSync(join(VUE_SRC, 'index.ts'), generateVueIndex(allIconNames))
    console.log(`     ✅ packages/vue/src/index.ts`)

    // tsup config
    writeFileSync(TSUP_CFG, generateTsupConfig(allIconNames))
    console.log(`     ✅ packages/core/tsup.config.ts`)
  } else {
    console.log('     (dry run — no files written)')
  }

  console.log(`\n🎉 Done! ${allIconNames.length} icons → ${allIconNames.length * 4} components`)
